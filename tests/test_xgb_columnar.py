"""Comprehensive round-trip tests for XGBoost columnar tensor serialization.

Tests prediction equivalence across regression, binary classification,
and multiclass classification, for both columnar-tensors and native-json
...
class TestBinaryClassifierRoundTrip:
    ...
    @pytest.mark.parametrize("config", CLASSIFIER_CONFIGS)
    def test_raw_margin(self, binary_Xy, config):
        X, y = binary_Xy
        model = xgb.XGBClassifier(**config)
        model.fit(X, y)
